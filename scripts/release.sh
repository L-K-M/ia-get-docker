...
# to build the Docker image and push it to GHCR (ghcr.io/l-k-m/ia-get-docker, tagged
# X.Y.Z, X.Y, and latest; :latest skipped for pre-releases like v1.2.3-rc.1). Nothing
# committed declares a version — the Dockerfile builds upstream ia-get from the
# IA_GET_REF *branch* (main) and CI's metadata-action derives every image tag from the
...
export RELEASE_APP_NAME="ia-get-docker"
export RELEASE_KIND="tag-only"
export RELEASE_CI_NOTE="CI (release.yml) will now build the Docker image and publish ghcr.io/l-k-m/ia-get-docker:<version> (stable releases also get :major.minor and :latest)."
