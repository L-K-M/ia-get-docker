...
ia-get-docker bundles the `ia-get` downloader with a Flask web UI. It ships as a
single Docker image built from the root [`Dockerfile`](Dockerfile) (a multi-stage
build that compiles `ia-get` from Rust, builds the `ui/` front-end with Node, and
serves it from a Python runtime) and is published to the GitHub Container
Registry (GHCR). This document describes the GitHub Actions workflows that check
the code, verify the image builds, and publish it.
...
- **Build Docker image** — builds the image with
  ...
  `Dockerfile`, which copies `requirements.txt`, `app/`, and `ui/`. The image
  exposes port `8080`. GitHub Actions layer caching (`type=gha`) keeps rebuilds
  fast.
...
The image is published to `ghcr.io/l-k-m/ia-get-docker` with tags `1.2.3`,
...
`L-K-M` the image path is `ghcr.io/l-k-m/ia-get-docker`. Pull with:

```
docker pull ghcr.io/l-k-m/ia-get-docker:1.2.3
```
